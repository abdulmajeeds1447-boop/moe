...
// تأكد من أنك تستخدم المكتبة الصحيحة. إذا واجهت مشاكل مع @google/genai، استخدم @google/generative-ai
...
المعايير والدرجة العظمى لكل معيار:
...
القواعد الصارمة:
- التقييم يتم مباشرة من أصل الدرجة العظمى (مثلاً 9 من 10، أو 4 من 5).
- خفض الدرجة إذا لم تجد شاهداً صريحاً للمعيار (إلى 0).
- لا تمنح الدرجة النهائية إلا بوجود أدلة كافية.

الرد يجب أن يكون JSON فقط يحتوي على الدرجات المقترحة والتبرير.
    `;

    // --- التعديل الجوهري هنا ---
    const response = await ai.models.generateContent({
      model: 'gemini-1.5-flash', // ✅ تم التصحيح: هذا هو الموديل الحقيقي والمتاح حالياً
    ...
    return NextResponse.json(JSON.parse(response.text() || '{}')); // تصحيح بسيط: response.text() هي دالة
    ...
    return NextResponse.json({ 
      ...
      details: error.message || 'تأكد من اسم الموديل وصلاحية المفتاح'
    }, { status: 500 });
  }
}
