...
const EvaluationModal: React.FC<EvaluationModalProps> = ({ submission, onClose, isViewOnly = false }) => {
  ...
  const runAIAnalysis = async () => {
    ...
    setIsAnalyzing(true);
    setAnalysisStatus('جاري تدقيق الشواهد بصرامة مئوية...');
    
    try {
      const data = await analyzeTeacherReport(submission.drive_link, (attempt) => {
        setAnalysisStatus(`إعادة محاولة الاتصال بالمدقق الذكي (${attempt}/3)`);
      });
  ...
  const saveEvaluation = async () => {
    ...
    try {
      ...
      alert('✅ تم حفظ التقييم المدقق بنجاح');
      onClose();
      ...
      <div className="print:hidden bg-white w-full max-w-6xl rounded-[3rem] shadow-2xl flex flex-col max-h-[96vh] overflow-hidden border border-white/20">
        <div className="p-6 bg-moe-navy text-white flex justify-between items-center relative overflow-hidden">
          ...
          <div className="flex items-center gap-4 relative z-10">
            ...
            <div>
              <h2 className="text-xl font-black">نظام التدقيق المئوي الصارم</h2>
              <p className="text-[10px] text-moe-teal font-bold uppercase tracking-[0.2em]">إدارة: نايف الشهري</p>
            </div>
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-12">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-widest">المعايير والدرجات</h3>
                <span className="text-[10px] font-black bg-slate-100 px-3 py-1 rounded-full text-slate-500">0 = لا يوجد شاهد</span>
              </div>
                  ...
                  <div key={c.id} className="p-4 bg-white rounded-2xl border border-slate-100 flex justify-between items-center group hover:border-moe-teal hover:shadow-md transition-all">
                    <div className="flex flex-col">
                      ...
                      <div className="flex gap-3 mt-1">
                        <span className="text-[9px] font-bold text-slate-400">الوزن: {c.weight}%</span>
            ...
            <div className="space-y-8">
              <div className="bg-gradient-to-br from-moe-navy to-[#0a2831] p-10 rounded-[2.5rem] text-white text-center shadow-xl relative overflow-hidden">
                ...
                <p className="text-xs font-bold opacity-60 mb-2 uppercase tracking-widest">إجمالي التقييم الرقمي</p>
                  ...
                  {!isViewOnly && (
                    <>
                      <button onClick={runAIAnalysis} className="col-span-2 py-5 bg-moe-teal text-white rounded-2xl font-black shadow-lg shadow-moe-teal/20 hover:brightness-110 active:scale-[0.98] transition-all">
                        ⚡ بدء التدقيق الذكي (Auditor Mode)
                      </button>
                      <button onClick={saveEvaluation} disabled={isSaving} className="py-5 bg-moe-navy text-white rounded-2xl font-black shadow-lg hover:bg-[#1a4a58] transition-all">
                        {isSaving ? 'جاري الحفظ...' : 'اعتماد وحفظ البطاقة'}
                      </button>
              ...
              <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm relative">
                <div className="absolute -top-3 right-8 bg-white px-4 py-1 border border-slate-100 rounded-full text-[10px] font-black text-slate-400">تقرير المدقق المرفق</div>
                <div className="w-full h-48 text-xs font-bold leading-relaxed bg-slate-50/50 p-6 rounded-2xl overflow-y-auto whitespace-pre-wrap text-slate-700 border border-slate-50">
                  {justification || 'سيظهر هنا تحليل المدقق الذكي بناءً على الشواهد التي يعثر عليها فقط...'}
                </div>
              </div>
            </div>
          </div>
