...
const EvaluationModal: React.FC<EvaluationModalProps> = ({ submission, onClose, isViewOnly = false }) => {
  ...
  const calculateTotal = () => {
    ...
    EVALUATION_CRITERIA.forEach(c => { 
      // الحصول على الدرجة من 5 (افتراضياً 0)
      const scoreOutOfFive = Number(scores[c.id] || 0);
      // الوزن النسبي (10 أو 5)
      const weight = Number(c.weight);
      // المعادلة: (الدرجة / 5) * الوزن
      // مثال: (4 / 5) * 10 = 8%
      totalPercentage += (scoreOutOfFive / 5) * weight;
    });
  ...
  const sendWhatsApp = () => {
    ...
    const reportJustification = justification || 'تم رصد الدرجات بناءً على الشواهد والأدلة الرقمية المرفوعة.';
    
    const message = `*تقرير تقييم الأداء الوظيفي الرقمي* 📄%0A%0A` +
      `*عزيزي المعلم/ة:* ${teacherName}%0A%0A` +
      `*تحية طيبة، مرفق لكم نتيجة تقييم الأداء:*%0A` +
      ...
      `*تبريرات التقييم (بناءً على الشواهد):*%0A${reportJustification}%0A%0A` +
          ...
          <table className="w-full border-collapse border border-slate-400 text-[8.5px]">
            ...
            <tbody>
              {EVALUATION_CRITERIA.map(c => {
                ...
                const weight = Number(c.weight);
                const calculatedScore = (score / 5) * weight;
                return (
                  <tr key={c.id}>
                    <td className="border border-slate-400 p-0.5 px-1.5 font-bold">{c.label}</td>
                    <td className="border border-slate-400 p-0.5 text-center">{weight}%</td>
                    <td className="border border-slate-400 p-0.5 text-center font-black">{calculatedScore}</td>
                  </tr>
                );
              })}
            ...
            <div className="space-y-8">
              ...
              {isAnalyzing ? (
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <button onClick={handlePrint} className={`py-5 bg-slate-100 text-moe-navy border-2 border-slate-200 rounded-2xl font-black transition-all hover:bg-white`}>
                    📄 طباعة التقرير
                  </button>
